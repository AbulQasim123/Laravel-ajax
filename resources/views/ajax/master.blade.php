			...
			$.ajax({
				...
				success: function(result){
					...
					for(var count = 0; count < result.length; count++){
						...
						html += '<td><button type="button" class="btn btn-info btn-sm edit" id="'+result[count].id+'">Edit</button>&nbsp;&nbsp;<button type="button" class="btn btn-danger btn-sm delete" id="'+result[count].id+'">Delete</button></td></tr>';
					}
		...
		function FetchInSelectBox(){
			$.ajax({
				...
				success: function(result){
					...
					for(var count = 0; count < result.length; count++){
						html += '<option value="'+result[count].id+'">'+result[count].firstname+'</option>';
					}
				...
				success: function(response){
					...
					if (states.length > 0) {
						for(var count = 0; count < states.length; count++){
							html += '<option value="'+states[count]['state_name']+'">'+states[count]['state_name']+'</option>';
						}
					}else{
						$('#cities').html('<option value="">Select City</option>');
					}
				...
				success: function(response){
					...
					if (cities.length > 0) {
						for(var count = 0; count < cities.length; count++){
							html += '<option value="'+cities[count]['city_name']+'">'+cities[count]['city_name']+'</option>';
						}
					}
					$('#cities').html(html);
				}
