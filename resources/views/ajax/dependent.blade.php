    ...
    <div class="container-fluid">
                            ...
                            <tbody>
                                @if (count($students) > 0)
                                    @foreach ($students as $student)
                                        <tr>
                                            <td><input type="checkbox" id="ids" name="ids[{{ $student->id }}]"
                                                    value="{{ $student->id }}"></td>
            ...
            <div class="col-md-6">
                ...
                <form id="DeleteUser">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover table-sm">
                            ...
                            <tbody>
                                @if (count($students) > 0)
                                    @foreach ($students as $student)
                                        <tr class="{{ $student->id }}">
                                            <td><input type="checkbox" id="del_id" name="del_id[]"
                                                    value="{{ $student->id }}"></td>
                                            <td>{{ $student->firstname }}</td>
                                            <td>{{ $student->lastname }}</td>
                                            <td>{{ $student->email }}</td>
                        ...
                        <input type="submit" value="Delete" id="delete_ajax" name="delete_ajax"
            ...
            <div class="col-md-6">
                ...
                <div class="form-group">
                    <form action="{{ route('upload.img') }}" method="post" enctype="multipart/form-data">
                        ...
                        <input type="submit" value="Upload" id="image" name="image"
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <div class="form-group">
                    <form id="imageupload">
                        ...
                        <button type="submit" class="btn btn-primary btn-sm" id="btn_upload_images">Upload</button>
            ...
            <div class="col-md-6">
                ...
                <div class="form-group">
                    <form method="post" action="{{ route('draganddrop.image') }}"
            ...
            <div class="col-md-6">
                ...
                <div>
                    <form action="{{ route('MultiSelect.data') }}" method="POST">
                        ...
                        <input type="text" id="enter_name" name="enter_name" class="form-control"
                            placeholder="Enter Name">
                        <label for="hobey">Select Hobey</label>
                        <select name="hobey[]" multiple="multiple" id="hobey" class="form-control">
                ...
                <div>
                    <form action="{{ route('Multicheckbox.data') }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="checkbox_name" name="checkbox_name" class="form-control"
                                placeholder="Enter Name">
                        </div>
                        <div class="form-group">
                    ...
                    @if (count($downloadimages) > 0)
                        @foreach ($downloadimages as $image)
                            <div style="display: inline-block; margin: 10px; padding: 5px;">
                                <img src="{{ asset('images/' . $image->path) }}"alt="Not Found" width="200"
                                    height="200" class="img-thumbnail"><br><br>

                                Download ( <span class="count_{{ $image['id'] }}">{{ $image['count'] }}
                                </span>)<br><br>

                                <a href="{{ 'public/' . $image['path'] }}" target="_blank"
                                    download="{{ $image['name'] }}" data-id="{{ $image['id'] }}" class="downloader">
        ...
        $(document).ready(function() {
                    ...
                    success: function(result) {
                        ...
                        if (result.length > 0) {
                            for (var i = 0; i < result.length; i++) {
                                html += '<tr><td>' + result[i]['firstname'] + '</td><td>' +
                                    result[i]
                                    ['lastname'] + '</td><td>' + result[i]['email'] +
                                    '</td><td>' + result[i]
                                    ['phone'] + '</td></tr>';
                            }
                        } else {
                            html +=
                                '<tr align="center"><td colspan="4">No Data Found.</td></tr>';
                        }
                        ...
                        if (result.length > 0) {
                            for (var i = 0; i < result.length; i++) {
                                html += '<tr><td>' + result[i]['name'] + '</td><td>' + result[i]
                                    ['price'] + '</td><td>' + result[i]['description'] +
                                    '</td></tr>';
                            }
                        } else {
                            html +=
                                '<tr align="center"><td colspan="3">No Data Found.</td></tr>';
                        }
            ...
            $('.downloader').click(function() {
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        // console.log(result);
                        $('.count_' + imgid).text(result.imgcount);
